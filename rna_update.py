...
class Update:
    ...
    def flist(self):
        ...
        for f in file_list:
            ...
            # if file name contains '.txt' at the end, analyze it
            if re.search('(\.txt$)', f):

                # remove '.txt' from label
                label = re.sub('(\.txt$)', '', f)
            ...
            if file_ok == True:
                # identify all rows for table RNA-Seq
                ...
                try:
                    # insert values into RNA-Seq table
                    c.execute('insert into RNAseq values (?, ?, ?, ?, ?)',
